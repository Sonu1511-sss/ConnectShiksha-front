...
const Journey = () => {
  return (
    <section id="journey" className="bg-white py-20 px-4 sm:px-8 lg:px-20 text-[#1E3A8A]">
      <h2 className="text-3xl md:text-4xl font-bold text-center mb-16">Our Journey</h2>

      <div className="relative max-w-6xl mx-auto">
        <svg className="absolute top-6 left-0 w-full h-12" viewBox="0 0 1000 100" preserveAspectRatio="none">
          <path d="M0,30 C150,70 350,-10 500,30 C650,70 850,-10 1000,30" fill="none" stroke="#FACC15" strokeWidth="4" />
        </svg>

        <div className="relative flex flex-col md:flex-row justify-between items-start pt-16 gap-12 md:gap-0">
          {milestones.map((item, index) => (
            <div key={index} className="group flex flex-col items-center relative w-full md:w-1/4">
              ...
              <p className="text-sm text-center text-gray-600 mt-1">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
